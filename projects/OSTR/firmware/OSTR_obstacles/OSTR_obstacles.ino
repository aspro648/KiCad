...
void loop(){ 
  ...
  // check situation
  bool wallLeft = !digitalRead(leftDetector);
  bool wallRight = !digitalRead(rightDetector);
  
  // react to what was detected
  moveBackward(10);
  if (wallRight && wallLeft){
    turnLeft(random(80, 180));
  }
  else if(wallRight){
    turnLeft(random(15, 90));
  }
  else if(wallLeft){
    turnRight(random(15, 90));
  }
  clear = true; 
}

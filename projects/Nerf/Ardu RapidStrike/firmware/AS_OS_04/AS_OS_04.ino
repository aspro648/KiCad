...
Ver 0.4 -> switching to use Hex Schmitt Inverter (TRIG, REV, PUSH, BUTTON should be reversed in logic)
...
void setup() {                  
  ...
  attachInterrupt(digitalPinToInterrupt(pusherSwitch), pusher_interrupt, RISING);
  ...
  if(!digitalRead(pusherSwitch)){
    ...
    while(!digitalRead(pusherSwitch)){1;
    }
